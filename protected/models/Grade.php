...
class Grade extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			...
			array('name, level', 'length', 'max'=>45),
			array('timestamp, update_date, create_time', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, name, level, timestamp, createby, update_date, create_time', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			'gradeSections' => array(self::HAS_MANY, 'GradeSection', 'grade_id'),
		);
	}
