...
class Messages extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			...
			array('sender_id, receiver_id, createby', 'numerical', 'integerOnly'=>true),
			array('subject', 'length', 'max'=>256),
			array('is_read', 'length', 'max'=>1),
			array('deleted_by', 'length', 'max'=>8),
			array('body, timestamp, update_date, create_time', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, sender_id, receiver_id, subject, body, is_read, deleted_by, created_at, timestamp, createby, update_date, create_time', 'safe', 'on'=>'search'),
		);
	}
